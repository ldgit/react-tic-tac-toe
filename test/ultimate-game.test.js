...
describe('playSquare', () => {
  ...
  it('changing board squares of new state should not alter previous state (immutability)', () => {
    const newState = playSquare(initialState, { boardIndex: 0, squareIndex: 0 });
  ...
  it('playing the same square twice will not advance to next player', () => {
    const midState = playSquare(initialState, { boardIndex: 0, squareIndex: 0 });
  ...
  it('playing the same square twice will not change the value of that square', () => {
    const midState = playSquare(initialState, { boardIndex: 1, squareIndex: 1 });
    ...
    const stateOne = playSquare(initialState, { boardIndex: 0, squareIndex: 4 });
  ...
  it('playing an occupied square on inactive board does nothing', () => {
    const midState = playSquare(initialState, { boardIndex: 8, squareIndex: '4' });
    const finalState = playSquare(midState, { boardIndex: 8, squareIndex: '4' });
  ...
  it('if move takes the player to won board, mark all boards as active (covers the case where move that takes the player to won board is also the one that won that board)', () => {
    const firstXMoveState = playSquare(initialState, { boardIndex: 0, squareIndex: 8 });
    const firstOMoveState = playSquare(firstXMoveState, { boardIndex: 8, squareIndex: 0 });
    const secondXMoveState = playSquare(firstOMoveState, { boardIndex: 0, squareIndex: 4 });
    const secondOMoveState = playSquare(secondXMoveState, { boardIndex: 4, squareIndex: 0 });
    // X wins board 0, O would otherwise need to play on that same board:
    const thirdXMoveState = playSquare(secondOMoveState, { boardIndex: 0, squareIndex: 0 });
  ...
  it('playing a move on inactive board that takes the player to a won board should do nothing', () => {
    ...
    const secondXMoveState = playSquare(firstOMoveState, { boardIndex: 0, squareIndex: 4 });
    const secondOMoveState = playSquare(secondXMoveState, { boardIndex: 4, squareIndex: 0 });
    const thirdXMoveState = playSquare(secondOMoveState, { boardIndex: 0, squareIndex: 0 }); // X wins board 0
    // Inactivates all boards except 8:
    const thirdOMoveState = playSquare(thirdXMoveState, { boardIndex: 1, squareIndex: 8 });

    // Move that takes the player to won board:
    const fourthXMoveState = playSquare(thirdOMoveState, { boardIndex: 7, squareIndex: 0 });
  ...
  context('history and time travel', () => {
    it('valid move should push new board state to history', () => {
      const newState = playSquare(initialState, { boardIndex: 1, squareIndex: 8 });
    ...
    it('valid move should increment pointInHistory in state', () => {
      ...
      currentState = playSquare(initialState, { boardIndex: 1, squareIndex: 8 });
      assert.equal(currentState.pointInHistory, 1);
      currentState = playSquare(currentState, { boardIndex: 8, squareIndex: 2 });
      assert.equal(currentState.pointInHistory, 2);
    });

    it('invalid move should not add more entries to history', () => {
      const stateBeforeInvalidMove = playSquare(initialState, { boardIndex: 1, squareIndex: 8 });
      const stateAfterInvalidMove = playSquare(stateBeforeInvalidMove, { boardIndex: 7, squareIndex: 4 });
    ...
    context('jump to a point in history', () => {
      it('should change pointInHistory property', () => {
        const newState = playSquare(initialState, { boardIndex: 1, squareIndex: 8 });
        const stateAfterTimeTravel = timeTravel(newState, { pointInHistory: 0 });
        assert.strictEqual(stateAfterTimeTravel.pointInHistory, 0);
      });

      it('should change next player accordingly', () => {
        const firstMoveState = playSquare(initialState, { boardIndex: 1, squareIndex: 8 });
        const secondMoveState = playSquare(firstMoveState, { boardIndex: 1, squareIndex: 8 });
      ...
      it(
        ...
        () => {
          const moveOneState = playSquare(initialState, { boardIndex: 4, squareIndex: 0 });
          const moveTwoState = playSquare(moveOneState, { boardIndex: 0, squareIndex: 2 });
          const moveThreeState = playSquare(moveTwoState, { boardIndex: 2, squareIndex: 3 });

          const stateAfterTimeTravelToMoveOne = timeTravel(moveThreeState, { pointInHistory: 1 });
          const finalState = playSquare(stateAfterTimeTravelToMoveOne, { boardIndex: 0, squareIndex: 3 });
      ...
      it('when time traveling to past, all rules related to that move apply', () => {
        ...
        const stateAfterTimeTravelToMoveOne = timeTravel(moveThreeState, { pointInHistory: 1 });
        const invalidMoveState = playSquare(stateAfterTimeTravelToMoveOne, { boardIndex: 1, squareIndex: 3 });
      ...
      it('Play one move, then back to game start, then repeat the move. Next player should be O.', () => {
        ...
        const repeatedMoveOneState = playSquare(backToGameStart, { boardIndex: 4, squareIndex: 0 });

        assert.equal(repeatedMoveOneState.nextPlayer, 'O');
      });

      it('should not delete history entries', () => {
        const midState = playSquare(initialState, { boardIndex: 1, squareIndex: '8' });
        const stateBeforeTimeTravel = playSquare(midState, { boardIndex: 8, squareIndex: '5' });
        const stateAfterTimeTravel = timeTravel(stateBeforeTimeTravel, { pointInHistory: 0 });
      ...
      it('after playing a new valid move, all history after that move is discarded', () => {
        const midState = playSquare(initialState, { boardIndex: 1, squareIndex: '8' });
        let stateBeforeTimeTravel = playSquare(midState, { boardIndex: 8, squareIndex: '5' });
        stateBeforeTimeTravel = playSquare(stateBeforeTimeTravel, { boardIndex: 5, squareIndex: '5' });

        const stateAfterTimeTravel = timeTravel(stateBeforeTimeTravel, { pointInHistory: 0 });
        const stateAfterValidMove = playSquare(stateAfterTimeTravel, { boardIndex: 8, squareIndex: '3' });
...
describe('deepCopyGameState', () => {
  it('should copy top level properties', () => {
    ...
    const afterTimeTravelState = timeTravel(initialState, { pointInHistory: 1 });
